...
module Notifications
  ...
  def build_notifications(message_type, message="")
   ...
   Notification.create!(item: self, user: notification_user, notification_type: :contact, message_type: message_type, message: message )
    ...
    agency_users = User.where(agency_id: agencies.map(&:id), agency_notifications: true).where("id NOT IN (?)",contact_user_ids)
    ...
    admin_users = User.where(role: User.roles[:admin]).where("id NOT in (?)",contact_user_ids)

    total_users = agency_users + admin_users
   
    total_users.uniq(&:email).each do |notification_user|
  ...
  def notifications_user_active(message="")
     ...
     Notification.create!(item: self, user: notification_user, notification_type: :contact, message_type: 'activate', message: 'This is a test message' )
     end
    contact_user_ids = contact_users.map(&:id)
 end
